package com.ants.modules.file.controller;
...
public class FileController {
    ...
    @ApiOperation("获取文件列表")
    @GetMapping("/get")
    public Result getAll(AntsFile antsFile,
    ...
    public Result thoroughDelete(@RequestParam String id) {
        boolean b = fileService.thoroughDelete(id);
        return Result.ok(b);
    }
    ...
    public Result<?> upload(HttpServletRequest request,
        ...
        if (CommonConstant.UPLOAD_TYPE_LOCAL.equals(uploadType)) {
            fileId = fileService.uploadLocal(file, uploadpath, uploadType, fileType);
        } else {
            fileId = fileService.upload(file, uploadpath, uploadType, fileType);
        }
        if (oConvertUtils.isNotEmpty(fileId)) {
            result.setMessage(fileId);
