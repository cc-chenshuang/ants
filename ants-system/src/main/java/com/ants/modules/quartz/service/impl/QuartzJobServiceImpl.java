package com.ants.modules.quartz.service.impl;
...
import com.ants.modules.quartz.mapper.QuartzJobMapper;
...
public class QuartzJobServiceImpl extends ServiceImpl<QuartzJobMapper, QuartzJob> implements IQuartzJobService {
	...
	private void schedulerAdd(String jobClassName, String cronExpression, String parameter) {
		try {
			...
			throw new AntsException("创建定时任务失败", e);
		} catch (RuntimeException e) {
			throw new AntsException(e.getMessage(), e);
		}catch (Exception e) {
			throw new AntsException("后台找不到该类名：" + jobClassName, e);
		}
	}
	...
	private void schedulerDelete(String jobClassName) {
		try {
			...
			throw new AntsException("删除定时任务失败");
		}
	}
