...
import com.ants.common.constant.FillRuleConstant;
import com.ants.common.exception.AntsException;
import com.ants.common.utils.FillRuleUtil;
import com.ants.common.utils.oConvertUtils;
import com.ants.modules.system.entity.SysCategory;
import com.ants.modules.system.mapper.SysCategoryMapper;
...
public class SysCategoryServiceImpl extends ServiceImpl<SysCategoryMapper, SysCategory> implements ISysCategoryService {
	...
	@Override
	public List<TreeSelectModel> queryListByCode(String pcode) throws AntsException {
		...
		if(oConvertUtils.isNotEmpty(pcode)) {
			...
			if(list==null || list.size() ==0) {
				throw new AntsException("该编码【"+pcode+"】不存在，请核实!");
			}
			if(list.size()>1) {
				throw new AntsException("该编码【"+pcode+"】存在多个，请核实!");
			}
